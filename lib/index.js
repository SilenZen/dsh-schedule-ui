...
// ==== 插件主体：为 UI 提供 HTTP 读写接口（与动态版 harness.handle 等价）====
export const name = 'schedule-ui';
export const inject = ['sessions', 'webServer'];
export function apply(ctx) {
	const tails = new Map();
	const runTx = (sessionId, operation) => {
		const run = (tails.get(sessionId) ?? Promise.resolve()).then(operation);
	...
	const err = (code, message) => Object.freeze({ ok: false, code, message });
	const ok = (value) => Object.freeze({ ok: true, ...value });
	const resolveSession = (sessionId) => {
		const session = ctx.sessions.get(sessionId);
		if (!session) return { error: err('session_not_found', 'This session is not live in the current process.') };
		return { session };
	};
	...
	const operations = {
		list: async (args) => {
			...
			return runTx(sessionId, async () => {
				if (!await flush(resolved.session)) return err('persistence_uncertain', 'Schedule persistence is uncertain; try again.');
				const folded = foldOf(resolved.session);
				if (folded === null) return err('corrupt_schedule_log', 'The session schedule log is corrupt.');
				const now = Date.now();
				return ok({ records: folded.active.map((record) => scheduleView(record, now)) });
			});
		},
		create: async (args) => {
			const sessionId = typeof args?.sessionId === 'string' ? args.sessionId : '';
			...
			return runTx(sessionId, async () => {
				...
				try {
					...
					if (args?.after_seconds !== void 0) record = createAfterScheduleRecord(id, prompt, args.after_seconds, Date.now());
					else if (args?.at !== void 0) record = createAtScheduleRecord(id, prompt, args.at, Date.now());
					else if (args?.every_seconds !== void 0) record = createEveryScheduleRecord(id, prompt, args.every_seconds, Date.now());
					else return err('invalid_selector', 'schedule.create accepts exactly one of after_seconds, at, or every_seconds.');
				} catch (error) {
					if (error instanceof ScheduleInputError) return err(error.code, error.message);
					return err('internal_error', 'The schedule operation failed.');
				}
				try {
					resolved.session.append('schedule/change', { version: 1, operation: 'create', schedule: record });
				} catch {
					return err('internal_error', 'The schedule operation failed.');
				}
				if (!await flush(resolved.session)) return err('persistence_uncertain', 'Schedule persistence is uncertain; try again.');
				return ok({ record: scheduleView(record, Date.now()) });
			});
		},
		delete: async (args) => {
			const sessionId = typeof args?.sessionId === 'string' ? args.sessionId : '';
			const id = typeof args?.id === 'string' ? args.id : '';
			if (id.length === 0 || id.trim() !== id) return err('invalid_rule', 'schedule.delete id must be non-empty without surrounding whitespace.');
			const resolved = resolveSession(sessionId);
			if (resolved.error) return resolved.error;
			return runTx(sessionId, async () => {
				if (!await flush(resolved.session)) return err('persistence_uncertain', 'Schedule persistence is uncertain; try again.');
				const folded = foldOf(resolved.session);
				if (folded === null) return err('corrupt_schedule_log', 'The session schedule log is corrupt.');
				if (!folded.active.some((record) => record.id === id)) return ok({ deleted: false, code: 'schedule_not_found' });
				try {
					resolved.session.append('schedule/change', { version: 1, operation: 'delete', id });
				} catch {
					return err('internal_error', 'The schedule operation failed.');
				}
				if (!await flush(resolved.session)) return err('persistence_uncertain', 'Schedule persistence is uncertain; try again.');
	...
	ctx.effect(() => {
		...
		const jsonRoute = (operation) => async (req, res) => {
			...
			try {
				...
				res.statusCode = 400;
				res.end(JSON.stringify({ ok: false, code: 'bad_request', message: 'The request body must be valid JSON.' }));
				return;
			}
			try {
				res.end(JSON.stringify(await operations[operation](args)));
			} catch {
				res.statusCode = 500;
				res.end(JSON.stringify({ ok: false, code: 'internal_error', message: 'The schedule operation failed.' }));
			}
		};
		const disposers = [
			ctx.webServer.register({ kind: 'exact', path: '/dsh-schedule-ui/list', handler: jsonRoute('list') }),
			ctx.webServer.register({ kind: 'exact', path: '/dsh-schedule-ui/create', handler: jsonRoute('create') }),
			ctx.webServer.register({ kind: 'exact', path: '/dsh-schedule-ui/delete', handler: jsonRoute('delete') }),
		];
		return () => {
